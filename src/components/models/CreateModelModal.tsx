...
export function CreateModelModal({
  ...
  const metadata = createdModel?.metadata as ModelMetadata | null;
  ...
  const handleGenerateFace = async () => {
    ...
    try {
      const res = await fetch(
        `/api/models/${createdModel.id}/generate-face`,
        { method: "POST" },
      );
      const result = await res.json();
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        ...
        className="!w-full sm:!w-[580px] !max-w-[calc(100%-2rem)] sm:!max-w-[580px] !rounded-[20px] !bg-[#18181B] !border !border-[#27272A] !px-4 sm:!px-[28px] !py-[20px] sm:!py-[24px] !gap-[16px] sm:!gap-[20px] !ring-0"
      >
        {/* Header */}
        ...
        {/* ─── STEP: PROFILE (show generated metadata) ─── */}
        {(step === "profile" || step === "generating-face" || step === "complete") && metadata && (
          <div className="flex flex-col gap-4 max-h-[50vh] sm:max-h-[420px] overflow-y-auto pr-1">
            {/* Face + basic info row */}
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-5">
              ...
              <div className="w-full sm:w-[140px] h-[200px] sm:h-[180px] rounded-[12px] overflow-hidden bg-[#27272A] shrink-0 flex items-center justify-center">
                {createdModel?.ref_face_url ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] text-[#52525B]">Generating...</span>
                  </div>
                ) : (
              ...
              <div className="flex-1 flex flex-col gap-2">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                  <InfoItem label="Height" value={`${metadata.height_cm} cm`} />
                  <InfoItem label="Weight" value={`${metadata.weight_kg} kg`} />
                  ...
                  <InfoItem label="Ethnicity" value={metadata.ethnicity_appearance} />
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <MeasureBadge label="Bust" value={`${metadata.bust_cm}cm`} />
              <MeasureBadge label="Waist" value={`${metadata.waist_cm}cm`} />
              <MeasureBadge label="Hips" value={`${metadata.hips_cm}cm`} />
            </div>
